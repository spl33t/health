...
ExecStart=/bin/bash -lc "set -euo pipefail; LOG_DIR='${LOG_DIR}'; mkdir -p \"\\\$LOG_DIR\"; TS=\\\$(date +%Y-%m-%d_%H-%M-%S); LOG_FILE=\"\\\$LOG_DIR/\\\${TS}.log\"; echo \"Logging to \\\$LOG_FILE\"; ${NODE_BIN} ${SCRIPT_DIR}/dist/src/index.js 2>&1 | tee -a \"\\\$LOG_FILE\""
Restart=always
RestartSec=5
